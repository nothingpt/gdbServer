  ...
  render () {
    ...
    return (
      <div className="container-newGDB">
        ...
        <div><span onMouseOver={ e => this.toggleHistory(e) } onMouseOut={ e => this.toggleHistory(e) } className='history-click'>Status: { status[status.length-1].statusType } <span className='status-date'>(<Moment format='DD/MM/YYYY'>{ status[status.length -1].statusDate }</Moment>)</span></span></div>
